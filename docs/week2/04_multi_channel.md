...
一张 32×32 RGB 图像在内存里是一个**三维张量**，形状 $(C, H, W) = (3, 32, 32)$（PyTorch 约定通道维在前）。每个通道都是一张独立的灰度图——R 通道记录红色亮度、G 记录绿色亮度、B 记录蓝色亮度，三张叠起来才合成你看到的彩色图。

把 CIFAR-10 第一张 horse（白马）实际拆开来看：
...
T2 里的 filter 是 2D 的 $k \times k$。**输入有 $C$ 个通道时，filter 必须也长成 $C \times k \times k$**——每个输入通道配一个独立的 $k \times k$ 子 filter，参数量从 $k^2$ 涨到 $C \cdot k^2$：

![filter 从 (3,3) 长成 (3, 3, 3)](../../assets/week2/figures/04_multi_channel/filter_shape_extension.png)

**关键点**：3 张子 filter 是**完全独立的**——不是同一个 2D filter 复制 3 份。网络可以学到"R 通道用 Sobel-x、G 通道用 Sobel-y、B 通道用 identity"这种**通道专属**的特征检测组合。
...
### 2.1.1 数值算例：每一步具体在算什么

抽象规则口头听过就忘。下面用一个**实际数字算例**走一遍——4×4 输入（3 通道）+ 3×3×3 filter → 2×2 输出。每个通道的输入都设计成不同的几何模式，每个子 filter 也专司一种检测：

| 通道 | 输入模式 | sub-filter | 期望响应 |
|---|---|---|---|
| R | 第 2 列是垂直亮条 | 检测垂直边（Sobel-x 简化）| 在亮条左侧给出强 + |
| G | 第 2 行是水平亮条 | 检测水平边（Sobel-y 简化）| 在亮条上方给出强 + |
| B | 主对角线 | identity 中心 | 直接抽取中心像素 |

逐通道算完，三张 2×2 中间结果对位相加即得最终输出 $Y$：

![多通道卷积数值算例](../../assets/week2/figures/04_multi_channel/multichannel_conv_numerical.png)

具体地：
- $\text{mid}_R = \begin{bmatrix}3 & 0\\ 3 & 0\end{bmatrix}$：左列 = R 通道左侧检测到亮条边缘
- $\text{mid}_G = \begin{bmatrix}3 & 3\\ 0 & 0\end{bmatrix}$：上行 = G 通道上方检测到亮条边缘
- $\text{mid}_B = \begin{bmatrix}1 & 0\\ 0 & 1\end{bmatrix}$：对角线 = B 通道 identity 抽出主对角
- $Y = \text{mid}_R + \text{mid}_G + \text{mid}_B = \begin{bmatrix}7 & 3\\ 3 & 1\end{bmatrix}$

**最值得注意的事**：$Y[0, 0] = 7$ 是因为这个位置在三个通道都有"事件"（R 的左边缘 + G 的上边缘 + B 的左上角），$Y[1, 1] = 1$ 是因为只有 B 通道在该位置有响应。**单 filter 的 $Y$ 是 3 个通道证据的"加权综合得分"**——这正是"多通道 filter = 跨通道证据加和"的物理含义。
...
![K=4 个 filter 各自卷积 → 沿通道堆叠](../../assets/week2/figures/04_multi_channel/K_filters_stack.png)

**关键关系**：**一层的输出通道数 = 这层的 filter 数 = 下一层的输入通道数**——这条链就是 CNN 通道数从 3 → 64 → 128 → … → 1024 一路加深的来源。每加一层卷积，通道数可以独立设计，与输入图像分辨率无关。
...
可视化对照（图里把三个张量并排画出来，能更清楚地看到 $C_{in}$ 在求和中"消失"、$C_{out}$ 由 $K$ 个独立 filter "撑出来"）：

![X / W / Y 三个张量的 4D 形状对照](../../assets/week2/figures/04_multi_channel/conv_tensor_shapes.png)
